...
TEST_P(AddSimpleTest, TestBuildModelAddSimple) {
  ...
  ASSERT_RESULT_OK_ASSIGN(
      ...
      litert::internal::graph_tools::GetSubgraph(model.get()));
  ASSERT_RESULT_OK_ASSIGN(
      auto subgraph_inputs,
  ...
  ASSERT_TRUE(litert::internal::graph_tools::ValidateTopology(ops));

  ASSERT_EQ(ops.size(), 1);
  auto op = ops[0];

  litert::internal::graph_tools::RankedTypeInfo float_2by2_type(
      kLiteRtElementTypeFloat32, {2, 2});
  ASSERT_TRUE(litert::internal::graph_tools::MatchOpType(
      op, {float_2by2_type, float_2by2_type}, {float_2by2_type},
      kLiteRtOpCodeTflAdd));

  ASSERT_RESULT_OK_ASSIGN(auto op_inputs,
                          litert::internal::graph_tools::GetOpIns(op));
  ASSERT_EQ(op_inputs.size(), 2);
  ASSERT_EQ(op_inputs[0], subgraph_inputs[0]);
  ASSERT_EQ(op_inputs[0], op_inputs[1]);

  ASSERT_RESULT_OK_ASSIGN(auto op_out,
                          litert::internal::graph_tools::GetOnlyOpOut(op));
  ASSERT_EQ(op_out, subgraph_outputs[0]);

  ASSERT_TRUE(
      litert::internal::graph_tools::MatchNoWeights(subgraph_outputs[0]));
  ASSERT_TRUE(
      litert::internal::graph_tools::MatchNoWeights(subgraph_inputs[0]));
}
...
TEST_P(AddCstTest, TestBuildModelAddCst) {
  ...
  ASSERT_EQ(op_inputs.size(), 2);
  ASSERT_EQ(op_inputs[0], subgraph_inputs[0]);
  ASSERT_TRUE(litert::internal::graph_tools::MatchWeights(
...
TEST_P(SimpleMultiOpTest, TestBuildModelSimpleMultiAdd) {
  ...
  ASSERT_TRUE(litert::internal::graph_tools::MatchOpType(
      ops[2], {float_2by2_type, float_2by2_type}, {float_2by2_type},
      kLiteRtOpCodeTflMul));
}
